...
| Ticket-autopilot static closure | `63,938` normalized UTF-8 bytes | `12` workflow files |
| Combined static prefix | `69,195` normalized UTF-8 bytes | Arithmetic sum of the two measured surfaces |
| Worst-case composed total | `176,851` normalized UTF-8 bytes | Static prefix plus the `107,656`-byte code-review volatile-input bound |
