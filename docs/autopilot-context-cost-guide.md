...
refreshed worktree-stable repository-authority and explicit migration guidance, and `WDT-01`
added canonical cross-checkout wiki delivery and exact local retry guidance. The configured
...
| Ticket-autopilot static closure | `67,081` normalized UTF-8 bytes | `11` workflow files |
| Combined static prefix | `72,361` normalized UTF-8 bytes | Arithmetic sum of the two measured surfaces |
| Worst-case composed total | `180,017` normalized UTF-8 bytes | Static prefix plus the `107,656`-byte code-review volatile-input bound |
