...
8. **Preserve human authority.** Mandatory workflow is not merge consent. Keep \`ticket-autopilot\` on its manual merge policy unless the user supplies the explicit durable authorization required by that skill. Never manufacture approval, credentials, provider evidence, or verification evidence. For a stale technical binding, load \`ticket-autopilot/references/technical-gates.md\`: distinguish human mandate, technical binding and operator store. Renew a covered binding with original provenance and remaining budget without duplicate consent; a scope change, revoked/expired mandate or exhausted budget is not a technical renewal. Preserve attempts and consumption, and never lift the user's runner suspension.
...
11. **Admit verification before spending.** Before checks or QA, load \`execute-ticket/references/verification-cost.md\`: validate the delivered packaging/artifact graph and candidate identity, select causal and mandatory checks, and account for all previous attempts/shards and remaining budget. Do not automatically repeat a complete suite because the candidate changed. A full repeat needs a specific causal or mandatory-policy reason and budget. Preserve failed attempts and original evidence identities; unknown cost is not zero. Never waive required full profiles or exact-head CI. This is a workflow requirement, not a claim of runtime shell interception.
