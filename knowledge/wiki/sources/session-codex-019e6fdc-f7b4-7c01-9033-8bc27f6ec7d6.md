...
# codex session 019e6fdc-f7b4-7c01-9033-8bc27f6ec7d6

A codex session recorded between 2026-05-28 to 2026-05-28, holding 452 records in 656,569 bytes. Everything below is what the session itself recorded doing; none of it is asserted as project truth, because a session's own account of its work can be wrong in exactly the ways the work was.
...
- Red is confirmed: the new module path is missing.
- I’m applying the same boundary split to tests that build evaluator contracts so the tests exercise the new public module directly instead of relying on a re-export.
