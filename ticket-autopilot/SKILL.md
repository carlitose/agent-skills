...
New `plan` and `run` invocations persist `--final-tree-mode off|observe|enabled`, defaulting to `observe`; historical missing configuration stays literal and malformed values fail closed. At the ordinary tracked completion boundary, observation mode builds a canonical content-addressed manifest from implementation CandidateRef `I`: exact ticket bytes/mode/digest, expected receipt, complete link closure, unique effects, full raw no-renames `I → D` rows, and no-extra-row proof; the unchanged finalizer then records parity or discrepancy. Enabled mode persists immutable intent before effects, applies each unique move/receipt/link once, reads each effect and the complete exact tree back, binds `D` only at `final-tree-bound`, and records `projected-not-integrated`. Exact prefixes resume after interruption and exact final replay returns `already-applied`; contradictions block without rollback, publication, provider mutation, integration, or pending-state restoration. This changes no final-quality scheduling yet. Neither mode transfers evidence, satisfies gates, or authorizes completion, publication, recovery, provider work, or merge. Excluded ignored/recovery/reconciliation/provider/drift/ambiguity/untracked/extra-effect cases retain the complete process; `status` exposes configuration, checkpoints, and all-false projection authority. For an ignored-source ticket whose exact candidate contains only a same-digest regular non-executable canonical `done/` receipt, `grant-completion-projection <run> --repo <repository> --ticket <id> --expected-tree <tree> --actor <identity> --evidence <durable-ref>` persists one immutable exact-repository/run/ticket/snapshot/CandidateRef/destination grant before resolving only its matching `source-mode-drift` gate. Validate bytes and mode from the Git index/tree plus the latest caller-owned source. Exact replay is idempotent; drift, contradiction, tracked base/current source, extra tracked source paths, or an unrelated gate fails closed. Candidate drift never inherits or retargets authority: a new exact actor/evidence invocation appends a successor after all immutable predecessors, and only the newest exact matching entry is active. Legacy singleton grants remain entry one; same-candidate contradictions and deleted, reordered, mutated, or branching lineage fail closed. A literal tracked-base gate remains blocked unless a lock-held, content-addressed proof binds it to the same run branch and runner-shaped prepared delivery commit, its parent/tree and ignored CandidateRef lineage, the newest grant, and a fresh terminal branch that lacks both the destination and head ancestry. Preserve the tracked observation on the gate; integrated, fetched, reconciled, arbitrary, stale, changed-branch, or multiple-gate cases remain forbidden. The grant log and delivery-head proof neither migrate ignored source ownership nor supply merge, provider, wiki-sync, finalization, or implementation-evidence authority, and descendants never inherit them.
