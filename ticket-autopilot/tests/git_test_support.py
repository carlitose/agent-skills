...
class GitIsolatedTestCase(unittest.TestCase):
    ...
    def repository_template(cls, build: Callable[[Path], None], *, name: str) -> Path:
        ...
        if name not in cache:
            ...
            cache[name] = template
            # Class cleanup ends the cache entry's lifetime as well as its directory's.
            # Another suite may reuse this class later in the same interpreter.
            cls.addClassCleanup(cache.pop, name, None)
        return cache[name]
